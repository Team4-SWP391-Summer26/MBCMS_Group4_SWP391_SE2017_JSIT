...
/**
 * DTO trao đổi qua WebSocket.
 * Serialize/deserialize thủ công bằng JSON string đơn giản
 * (không cần Jackson trong WebSocket endpoint).
 *
 * Format JSON:
 * {
 *   "action"     : "SELECT" | "DESELECT" | "HARD_LOCK" | "HARD_RELEASE",
 *   "seatId"     : 123,
 *   "showtimeId" : 456,
 *   "username"   : "userA"
 * }
 *
 * SELECT      = user click chọn ghế (soft lock)
 * DESELECT    = user bỏ chọn ghế   (release soft lock)
 * HARD_LOCK   = booking PENDING đã tạo (từ server broadcast)
