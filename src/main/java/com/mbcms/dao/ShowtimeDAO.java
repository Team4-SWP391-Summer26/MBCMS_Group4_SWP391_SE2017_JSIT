...
/**
 * ShowtimeDAO - truy van bang `showtimes`. Owner: HungNT (UC20 Schedule
 * showtimes).
 */
public interface ShowtimeDAO {

    /**
     * Lay showtime cua 1 branch (JOIN movies + rooms, kem so ghe da dat). Cac
     * filter movieId/roomId/date la tuy chon (null = khong loc) - SRS 3.5.2.1.
     */
    List<Showtime> findByBranch(long branchId, Long movieId, Long roomId, LocalDate date);

    /**
     * Tao showtime moi TRONG 1 TRANSACTION: check trung lich (overlap cung
     * phong) -> rong moi INSERT -> commit. UNIQUE(room_id, start_time) trong DB
     * la luoi an toan thu 2.
     *
    ...
    /**
     * Tim showtime theo id (JOIN movies + rooms nhu findByBranch); null neu
     * khong ton tai.
     */
    Showtime findById(long showtimeId);

    /**
     * Cap nhat showtime TRONG 1 TRANSACTION (UC21 Edit showtime). Check trung
     * lich nhu create nhung LOAI TRU CHINH NO (showtime_id <> ?) de sua suat ma
     * khong doi gio van luu duoc.
     *
